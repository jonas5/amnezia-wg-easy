...
module.exports = class WireGuard {
  ...
  async __buildConfig() {
    const { WG_HOST, WG_PATH, WG_DEFAULT_ADDRESS, JC, JMIN, JMAX, S1, S2, H1, H2, H3, H4 } = Settings.getAll();
  ...
  async __saveConfig(config) {
    const { WG_PORT, WG_PRE_UP, WG_POST_UP, WG_PRE_DOWN, WG_POST_DOWN, WG_PATH } = Settings.getAll();
    let result = `
...
Endpoint = ${peer.endpoint}:${WG_PORT}`;
  ...
  async getClientConfiguration({ clientId }) {
    const { WG_DEFAULT_DNS, WG_MTU, WG_ALLOWED_IPS, WG_PERSISTENT_KEEPALIVE, WG_HOST, WG_CONFIG_PORT } = Settings.getAll();
    const config = await this.getConfig();
...
Endpoint = ${WG_HOST}:${WG_CONFIG_PORT}`;
  ...
  async createClient({ name, expiredDate }) {
    const { WG_DEFAULT_ADDRESS } = Settings.getAll();
    if (!name) {
  ...
  async syncMesh() {
    const { MESH_ENABLED, MESH_PEERS, MESH_API_KEY } = Settings.getAll();
    if (!MESH_ENABLED) {
      return;
    }
  ...
  async cronJobEveryMinute() {
    const { WG_ENABLE_EXPIRES_TIME, WG_ENABLE_ONE_TIME_LINKS } = Settings.getAll();
    const config = await this.getConfig();
